...
DO $$
BEGIN
  -- Public can read support events for valid certifications
  IF NOT EXISTS (
  ...
  -- Users can read their own support events (as promoter or supporter)
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'promotion_support_events' 
  ...
  -- Creators can read support events for their own certifications
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'promotion_support_events' 
  ...
  -- Authenticated users can insert support events
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'promotion_support_events' 
